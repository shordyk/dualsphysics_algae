...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartFixed -onlytype:-all,+fixed -vars:-all -files:0
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartBeam -onlytype:-all,+floating
if [ $? -ne 0 ] ; then fail; fi
...
${floatinginfo} -dirin ${diroutdata} -onlymk:29 -savedata ${dirout2}/_FtInfo
if [ $? -ne 0 ] ; then fail; fi
