...
${measuretool} -dirin ${diroutdata} -pointsdef:ptels[x=2:1:80,y=2:1:38,z=-10:0.05:0] -onlytype:-all,+fluid -savevtk ${dirout2}/WaterBottom -height -vars:-all -files:0 -hvars:-all,bottom
if [ $? -ne 0 ] ; then fail; fi
${measuretool} -dirin ${diroutdata} -pointsdef:ptels[x=2:1:80,y=2:1:38,z=-10:0.05:1.5] -onlytype:-all,+fluid -savevtk ${dirout2}/WaterSurface -height -vars:-all -hvars:depthsum
if [ $? -ne 0 ] ; then fail; fi
